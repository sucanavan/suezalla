...
The `suezalla` package automatically attempts to load custom fonts when attached:

- **Roboto** is downloaded from Google Fonts and used in `theme_suezalla()`.
- **XKCD** is loaded from your system font folder (e.g., `~/Library/Fonts/`, `~/.fonts/`, or Windows font directory) if `xkcd.ttf` is present.

If the XKCD font is missing, the package will issue a startup message and fall back to system defaults.
